## Variables for ansiblenginx configuration
...
nginx_gzip_proxied: expired nocache nostore private auth
nginx_gzip_vary: 'on'
...
```
nginx_plus_upstream_enable_cookie:
nginx_plus_upstream_cookie_namel
nginx_plus_upstream_cookie_expires:
nginx_plus_upstream_cookie_domain:
nginx_plus_upstream_cookie_path:
nginx_plus_upstream_sticky_http_only:
nginx_plus_upstream_sticky_secure:
nginx_plus_upstream_sticky_route:
nginx_plus_upstream_sticky_learn:
nginx_plus_upstream_sticky_learn_create:
nginx_plus_upstream_sticky_learn_lookup:
nginx_plus_upstream_sticky_learn_zone:
nginx_plus_upstream_sticky_learn_timeout:
```
#### default global proxy cache configurations
```
nginx_plus_global_proxy_cache_path:
nginx_plus_global_proxy_cache_path_name:
nginx_plus_global_proxy_cache_keys_zone:
nginx_plus_global_proxy_cache_inactive:
nginx_plus_global_map_proxy_cache_purge_enabled:
nginx_plus_global_map_proxy_cache_purge:
nginx_plus_global_map_proxy_cache_default:
```
#### default server block proxy cache configurations
```
nginx_plus_status_block_enabled:
nginx_plus_status_sb_listen_port:
nginx_plus_status_sb_status_zone:
nginx_plus_status_sb_root:
nginx_plus_status_sb_location:
nginx_plus_status_sb_status_json:
nginx_plus_status_sb_status_allow:
nginx_plus_status_sb_status_deny:
```
#### default server block which will server / with http content to the upstream defined above
```
nginx_plus_default_sb_listen_port:
nginx_plus_default_sb_status_zone:
nginx_plus_default_sb_server_name:
nginx_plus_default_sb_location:
nginx_plus_default_sb_set_host_header:
nginx_plus_default_sb_set_real_ip:
nginx_plus_default_sb_set_x_forwarded:
nginx_plus_default_sb_proxy_pass:
nginx_plus_default_sb_proxy_cache:
nginx_plus_default_sb_health_check:
nginx_plus_default_sb_health_interval:
nginx_plus_default_sb_health_fails:
nginx_plus_default_sb_health_passes:
nginx_plus_default_sb_health_match_uri:
nginx_plus_default_sb_health_match_name:
nginx_plus_default_sb_status_json:
nginx_plus_default_sb_status_allow:
nginx_plus_default_sb_status_deny:
nginx_plus_default_sb_upstream_conf:
nginx_plus_default_sb_upstream_allow:
nginx_plus_default_sb_upstream_deny:
```
#### default tcp server block (rabbitmq)
```
nginx_plus_default_tcp_block_enabled:
nginx_plus_default_tcp_listen_port:
nginx_plus_default_tcp_status_zone:
nginx_plus_default_tcp_proxy_pass:
nginx_plus_default_tcp_upstream_name:
nginx_plus_default_tcp_zone_name:
nginx_plus_default_tcp_zone_size:
nginx_plus_default_tcp_server1:
```
